...
exports.scrapeProductData = async function (url, productSelectors) {
    ...
    const data = await page.evaluate((productSelectors) => {
        ...
        const productTitle = document.querySelector(productSelectors.titleSelector).innerText;

        let productAvailability = document.querySelector(productSelectors.availabilitySelector);
        productAvailability = productAvailability != null ? productAvailability.innerText.trim() : "No";

        if (productAvailability == "No") {
            return {productTitle, productAvailability,}
        }
        const productPrice = document.querySelector(productSelectors.priceSelector).innerText;

        const productImage = document.querySelector(productSelectors.imageSelector).src;

        return { productTitle, productPrice ,productAvailability, productImage}
